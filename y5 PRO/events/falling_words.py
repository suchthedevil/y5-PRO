...
c.pack()
letter = " "
words = ['jablko', 'programovanie', 'koleso', 'uloha', 'riesenie'] 
def key_pressed(e):
    global letter
    letter = e.char
...
def click(e):
    ...
    if game_end:
        ...
        word = words[k]
        text = "*"*len(word)
        id = c.create_text(w//2,30,text=text,font="arial 20")
        timer()
...
def timer():
    ...
    if text != word:
        ...
        if c.coords(id)[1] > h-5:
            c.delete(id)
            c.create_text(w//2,h//2,text="You lost!", font="arial 40", fill='red')
            game_end = True
    else:
        k += 1
        if k == len(words):
            c.delete(id)
            c.create_text(w//2,h//2,text="You won!", font="arial 40", fill='lime')
            game_end = True
        else:
            letter = " "
            word = words[k]
